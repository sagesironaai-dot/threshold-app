...
OWNS Synthesis cycle — the full MTM operation from lens page read through Finding production Claude API call for synthesis — system prompt assembly, data payload construction, response handling Lens page data assembly — pulling and structuring all five lens page datasets before the API call Finding production — extracting, validating, and writing Finding records from the Claude API response Content fingerprinting — generating and writing the content\_fingerprint on every Finding record Deduplication check on retry — comparing candidate Findings against prior session Findings before writing or routing anything LNV routing handoff — assembling the result object returned to DNR. MTM's responsibility ends there. synthesis\_sessions IDB store findings IDB store

DOES NOT OWN Lens page deposit writing — owned by their respective sections (THR · STR · INF · ECR · SNM) IDB reads of lens page entry data — owned by data.js LNV deposit writing — owned by LNV Routine orchestration and trigger — owned by DNR. MTM never calls itself. DNR calls MTM. WSC — Witness Scroll is sovereign. MTM has no relationship to it. Tag pipeline — owned by tagger\_bus.js Routing authority — owned by SOT Findings content after handoff — once the result object is returned to DNR, MTM does not track what LNV does with what it receives lnv\_routing\_status and lnv\_deposit\_id writes — after DNR confirms LNV has received a Finding, data.js writes lnv\_routing\_status → deposited and populates lnv\_deposit\_id on the findings record. DNR triggers this write. MTM does not.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ STRUCTURAL RULES ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. MTM never receives deposits. Synthesis only.

2. Synthesis cycle fires when MTM.runSynthesis() is called by DNR at session close only. Never triggered by deposit events. Never self-triggered.
...
DNR calls MTM.runSynthesis() and awaits its resolution before proceeding to Step 2\. MTM.runSynthesis() is the sole public entry point into this system. Everything inside MTM is internal to that call.
...
All five datasets are present in a single API call payload. If any dataset is missing because the section has no entries: the dataset is an empty array. An empty array is not a missing lens page. Synthesis proceeds. A lens page is only unavailable if IDB read fails for that section. That failure triggers pre\_synthesis abort.
...
Every Finding record carries a content\_fingerprint. Generated at Finding production time, before IDB write. Written to the findings record alongside all other fields.
...
Runs on every retry where prior\_mtm\_session\_id is present in the MTM.runSynthesis() call options. Never runs on a clean first-time synthesis — prior\_mtm\_session\_id is null on clean runs and the check is skipped entirely.
...
2. For each candidate Finding extracted from the Claude API response: a. Generate its content\_fingerprint. b. Check against prior\_fingerprints. c. If match found: skip. Do not write. Do not route. Log the skip — fingerprint, Finding title, prior session id. d. If no match: write to IDB, add fingerprint to prior\_fingerprints, proceed.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ MTM.runSynthesis() RESULT OBJECT ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
lens\_pages\_read — array of page codes Confirms which of the five lens pages were read in this cycle. Expected: \[THR, STR, INF, ECR, SNM\] If any IDB read failed before synthesis began: status → failed, failure\_type → pre\_synthesis.
...
Fires when DNR calls MTM.runSynthesis(options?). options.prior\_mtm\_session\_ids is present on retry runs. options is null or empty on clean first-time runs.
...
2. Read all five lens pages simultaneously from IDB: THR (02) · STR (03) · INF (04) · ECR (05) · SNM (06) Write lens\_pages\_read. If any IDB read fails: Write status → failed. Write failure\_type → pre\_synthesis. Return result object. Halt.
...
MTM.runSynthesis(options?) → Promise<ResultObject> The sole public entry point. Called by DNR only. Never called directly from any other system.
...
Returns: result object as defined in MTM.runSynthesis() RESULT OBJECT section. Always resolves — never throws to the caller. Failures are captured in the result object, not propagated as exceptions.
...
1. LENS PAGE IDB READ FAILS BEFORE SYNTHESIS One or more lens pages unavailable. Synthesis cannot run. No Findings produced. Guard: if any section IDB read fails at step 2, status → failed, failure\_type → pre\_synthesis. Result object returned immediately. DNR surfaces failure to LNV. Retry available.
...
6. MTM.runSynthesis() CALLED OUTSIDE DNR MTM fires without session context. No routine\_session record exists for this run. DNR does not receive the result. Findings may be written to IDB with no routing path to LNV. Provenance chain broken. Guard: MTM.runSynthesis() is the only public entry point. All other internal functions are unexposed. DNR is the only caller. Document this constraint explicitly at build. Do not expose additional entry points for convenience.
...
mtm.js MTM synthesis engine — lens page data assembly, Claude API call, JSON response handling, Finding validation, content fingerprinting, deduplication check, synthesis\_session and findings IDB writes, result object assembly. Status: PLANNED

data.js synthesis\_sessions IDB store — record creation, status writes, findings\_count write. findings IDB store — record creation, fingerprint write, lnv\_routing\_status tracking. Status: PLANNED
